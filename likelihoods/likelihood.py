...
class _Likelihood(Module, ABC):
    ...
    def _draw_likelihood_samples(self, function_dist, r_dist, seg_bag_idx, bag_labels_gt, video_names, *args, sample_shape=None, **kwargs):
        ...
        z_samples = torch.zeros(settings.num_likelihood_samples.value(), n_videos, n_segments)
        if torch.cuda.is_available():
            z_samples = Variable(z_samples).cuda()
        for i, video in enumerate(video_names):
            z_samples[:, i, :] = torch.nn.functional.one_hot(torch.multinomial(r_dist[i], settings.num_likelihood_samples.value(), replacement = True), num_classes = n_segments)
        
        
        function_samples = function_dist.rsample(sample_shape)
        
        return self.forward(function_samples, z_samples, seg_bag_idx, bag_labels_gt, video_names, *args, **kwargs)

    def expected_log_prob(self, function_dist, r_dist, seg_bag_idx, bag_labels_gt, video_names,  *args, **kwargs):
        
        [likelihood_samples, exp_log_prob] = self._draw_likelihood_samples(function_dist, r_dist, seg_bag_idx, bag_labels_gt, video_names, *args, **kwargs)
        # In this step it selects logp(y=GT|f); mean is taken across simulations
        res = likelihood_samples.mean(dim=0)
        exp_log_prob = exp_log_prob.mean(dim = 0)
        return [res, exp_log_prob]
