...
def evaluate(model, weights, best_auc_val, best_auc_test, aucs_val, aucs_test, split,\
        ...
        if best_auc_val<auc_val:
            ...
            torch.save({'state_dict': model.state_dict(), 
                        'opt_dict': optimizer.state_dict(),}, os.path.join("trained_models/SanghaiTech/model_"+str(rep)+"_"+str(split)+"_"+str(eps)+".pth.tar"))
            torch.save(weights, "trained_models/SanghaiTech/likelihood_weight_"+str(rep)+"_"+str(split)+"_"+str(eps)+".pt")
...
if __name__=="__main__":
    ...
    X_train_abnormal, X_train_normal = np.load("Dataset/SanghaiTech/X_train_abnormal.npy"), np.load("Dataset/SanghaiTech/X_train_normal.npy")
    ...
    pretrained = torch.load("pretrained_sanghaitech/model_top1_"+str(split)+".pth.tar", map_location = lambda storage, loc: storage)
    ...
    for i in range(max_iterations):
        ...
        if i%10==0:
            ...
            np.save("logs/SanghaiTech/aucs_eval_"+str(rep)+"_"+str(split)+"_"+str(eps)+".npy", aucs_eval)
            np.save("logs/SanghaiTech/aucs_test_"+str(rep)+"_"+str(split)+"_"+str(eps)+".npy", aucs_test)
            np.save("logs/SanghaiTech/losses_"+str(rep)+"_"+str(split)+"_"+str(eps)+".npy", losses)
            np.save("logs/SanghaiTech/kl_loss_z_"+str(rep)+"_"+str(split)+"_"+str(eps)+".npy", kl_z_loss)
            np.save("logs/SanghaiTech/kl_loss_u_"+str(rep)+"_"+str(split)+"_"+str(eps)+".npy", kl_u_loss)
            np.save("logs/SanghaiTech/exp_prob_loss_"+str(rep)+"_"+str(split)+"_"+str(eps)+".npy", exp_log_prob_loss)
            print("Working on an iteration")
            print("Best AUC Evaluation Score", best_auc_eval)
            print("Best AUC Testing Score", best_auc_test)
